...
Only the default plugins are generated — plugins mounted at runtime via `startGateway({ extraPluginDirs })` live outside the repo and document their own routes. The [`check:plugin-reference-fresh`](ia.md#checkplugin-reference-fresh) gate keeps the generated tables honest, shipping as the warn-only [`docs-freshness`](../../../.github/workflows/docs-freshness.yml) workflow. It annotates rather than blocks, so regenerate with `npm run regenerate-docs` and commit when a default plugin's routes change.
