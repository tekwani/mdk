 ...
 * postcss-mdk-layer
 *
 * Wraps every top-level rule that isn't already inside an `@layer` at-rule into a
 * single trailing `@layer mdk` block, and prepends the layer order declaration
 * `@layer base, mdk, app;` so consumers can override these styles from their own
 * `app` (or unlayered) styles without specificity wars.
 *
 * Kept in sync with `packages/react-devkit/postcss-mdk-layer.mjs` — the devkit
 * does not export it, and both stylesheets must land in the same `mdk` layer or
 * the cascade order between them becomes load-order dependent.
 */
