...
shape is `mdk-contract.schema.json` (in this directory; validate with
`../scripts/validate-contract.mjs`); this file covers the semantics the schema
can't express. Real contracts to imitate:
`backend/workers/samples/demo-worker/plugin/mdk-contract.json` (minimal),
`backend/workers/miners/whatsminer/plugin/mdk-contract.json` (full-featured),
`backend/workers/temperature/seneca/plugin/mdk-contract.json` (read-only
sensor).
...
- **Bounds rule:** every `number` param declares `min` and `max`. The Kernel's
  ...
  declared bounds*
  (`backend/core/kernel/lib/modules/command-dispatcher/index.js`); nothing
...
```
node ../scripts/validate-contract.mjs <worker>/plugin/mdk-contract.json
```
...
(`backend/workers/scripts/generate-catalogue.js`) separately lints shipped
